...
def test_repeat_roller():
    ...
    """
    diffs = diff(Game().play, path="tests/version2/repeat_roller.sim.txt")
...
def test_hot_dice():
    ...
    """
    diffs = diff(Game().play, path="tests/version2/hot_dice.sim.txt")
    assert not diffs, diffs
...
def test_cheat_and_fix():
    ...
    diffs = diff(Game().play, path="tests/version2/cheat_and_fix.sim.txt")
    assert not diffs, diffs
...
def test_zilcher():
    ...
    diffs = diff(Game().play, path="tests/version2/zilcher.sim.txt")
    assert not diffs, diffs